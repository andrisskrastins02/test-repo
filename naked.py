...
if r.status_code == 200: #Kods turpināsies ja r.statusa kods būs 200.
			...
			for val in json_data['near_earth_objects'][request_date]: #Cikls kurš izmanto json datu atslēgas "near_earth_objects" vērtības konkrētam pieprasījuma datumam.
				if 'name' and 'nasa_jpl_url' and 'estimated_diameter' and 'is_potentially_hazardous_asteroid' and 'close_approach_data' in val: #Parbauda, vai "val" ir pieejamas atslēgas (name, nasa_jpl_url, estimated_diameter, is_potentially_hazardous_asteroid, close_aproach_data), ja ir visas šīs atslēgas tad tiks izpildīts koda bloks.
					...
					if len(val['close_approach_data']) > 0: #Pārbauda vai asteroīdu close_approach_data ir lielāks par 0.
						...
						print("No close approach data in message") #Ja nav bijis tuvs asteroīds tad tiek izvadīts šis paziņojums.
					...
					print("------------------------------------------------------- >>") #Izdrukā līniju.
					print("Asteroid name: " + str(tmp_ast_name) + " | INFO: " + str(tmp_ast_nasa_jpl_url) + " | Diameter: " + str(tmp_ast_diam_min) + " - " + str(tmp_ast_diam_max) + " km | Hazardous: " + str(tmp_ast_hazardous)) #Izdrukā informāciju par asteroīdu.
					print("Close approach TS: " + str(tmp_ast_close_appr_ts) + " | Date/time UTC TZ: " + str(tmp_ast_close_appr_dt_utc) + " | Local TZ: " + str(tmp_ast_close_appr_dt)) #Izdrukā laiku kad asteroīds būs tuvu. 
					print("Speed: " + str(tmp_ast_speed) + " km/h" + " | MISS distance: " + str(tmp_ast_miss_dist) + " km") #Izdruka asteroīda ātrumu un miss distanci.
	...
	print("Hazardous asteorids: " + str(len(ast_hazardous)) + " | Safe asteroids: " + str(len(ast_safe))) #Izprintē nedrošo asteroīdu skaitu un drošo asteroīdu skaitu.
